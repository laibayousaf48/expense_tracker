...
public function store(Request $request)
{
    ...
    //     // Step 2: Fetch budgets for calculating remaining amounts
    //     $budgets = Budget::where('user_id', $userId)->get()->keyBy('category_id');
    
    //     // Step 3: Prepare analytics data
    //     $analyticsData = $categorizedExpenses->map(function ($expenses, $categoryName) use ($budgets) {
    //         // Calculate total expense amount for the category
    //         $totalAmount = $expenses->sum('amount');
    
    //         // Get the budget amount for the category, if it exists
    //         $category = $expenses->first()->category;
    //         $budgetAmount = $budgets[$category->id]->amount ?? 0;
    
    //         // Calculate remaining budget
    //         $remainingBudget = max($budgetAmount - $totalAmount, 0);
